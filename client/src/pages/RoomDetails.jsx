...
const RoomDetails = () => {
    ...
    const checkAvailability =async () => {
        try {
            //Check is check-In date is > than check out date
            if(checkInDate>=checkOutDate){
                toast.error('Check-In Date should be less than Check-Out Date')
                return;
            }
            const {data} = await axios.post('/api/bookings/checkAvailability', {room: id, checkInDate, checkOutDate})
            if(data.success){
                if(data.isAvailable){
                    setIsAvailable(true)
    ...
    return room && (
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
                ...
                <button
                    type="submit"
                    className="bg-teal-600 hover:bg-teal-700 active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer"
                >
            ...
            <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
                <p>Guests will be allocated on the ground floor according to availability. You get a comfortable Two bedroom apartment has a true city feeling. The price quoted is for two guest, at the guest slot please mark the number of guests to get the exact price for groups.</p>
            </div>
            ...
            <div className="flex flex-col items-start gap-4">
                <div className="flex items-start gap-4">
                    ...
                    {/* Host Image */}
                    <img
                        src="https://img.freepik.com/premium-photo/hotel-manager-isolated-cartoon-character_934475-17634.jpg"
                        alt="Host"
                ...
                <button className='px-6 py-2.5 mt-4 rounded text-white bg-teal-600 hover:bg-teal-700 transition-all cursor-pointer'>Contact Now</button>
            </div>
